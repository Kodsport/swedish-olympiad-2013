...
ll solve(map<pair<bool, ll>,ll>& memo, vector<ll> state, ll t)
{
    chomp(state);
    if (state.size()==0)
    {
    ...
    pair<bool, ll> mem = { state[0], t };
    if (state.size()==1 && setcontains(memo,mem))
    {
        return memo[mem];
    }
