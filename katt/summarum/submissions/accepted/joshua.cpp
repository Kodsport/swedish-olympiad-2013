...
int main()
{
    ...
    rep(i, n)
    {
        ...
        int j = 0;
        for (auto it = bestMatch; it != be.end() && j < 10; it = next(bestMatch,j), j++)
        {
            ans = min(ans, abs(sumAn - sumBe - 2 * an[i] + 2 * (*it)));
        }
        j = 0;
        for (auto it = bestMatch; it != be.end() && it != be.begin() && j < 10; it = prev(bestMatch,j), j++)
        {
            ans = min(ans, abs(sumAn - sumBe - 2 * an[i] + 2 * (*it)));
        }
        if (bestMatch == be.end())
